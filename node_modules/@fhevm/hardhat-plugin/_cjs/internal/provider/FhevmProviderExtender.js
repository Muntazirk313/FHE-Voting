...
class FhevmProviderExtender extends plugins_1.ProviderWrapper {
    ...
    async _handleFhevmRelayerV1PublicDecrypt(args) {
        ...
        const { decryptedResult, signatures } = await fhevmEnv.decryptionOracle.createDecryptionSignatures(payload.ciphertextHandles, clearTextHexList);
        const response = {
    ...
    async _handleFhevmCreateDecryptionSignatures(args) {
        ...
        const payload = _getRequestSingleParam(args);
        const res = await fhevmEnv.decryptionOracle.createDecryptionSignatures(payload.handlesBytes32Hex, payload.clearTextValuesHex);
        return res;
    }
    ...
    async _handleFhevmRelayerV1UserDecrypt(args) {
        ...
        await mock_utils_1.MockFhevmInstance.verifySignature(payload.publicKey, payload.signature, payload.contractAddresses, payload.userAddress, payload.requestValidity.startTimestamp, payload.requestValidity.durationDays, fhevmEnv.getGatewayDecryptionAddress(), Number(payload.contractsChainId));
    ...
    async _handleFhevmRelayerV1InputProof(args) {
        ...
        const response = await fhevmEnv.coprocessor.computeCoprocessorSignatures(handlesBytes32List, contractChainId, payload.contractAddress, payload.userAddress);
