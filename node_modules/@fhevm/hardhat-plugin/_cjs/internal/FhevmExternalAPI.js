...
class FhevmExternalAPI {
    ...
    async userDecryptEbool(handleBytes32, contractAddress, user, options) {
        ...
        const decryptedResults = await _userDecryptHandleBytes32(this._fhevmEnv.instance, [{ handleBytes32, contractAddress: addr, fhevmType: mock_utils_1.FhevmType.ebool }], user, options);
    ...
    async userDecryptEuint(fhevmType, handleBytes32, contractAddress, user, options) {
        const addr = await ethers_1.ethers.resolveAddress(contractAddress);
        const decryptedResults = await _userDecryptHandleBytes32(options?.instance ?? this._fhevmEnv.instance, [{ handleBytes32, contractAddress: addr, fhevmType }], user, options);
    ...
    async userDecryptEaddress(handleBytes32, contractAddress, user, options) {
        const addr = await ethers_1.ethers.resolveAddress(contractAddress);
        const decryptedResults = await _userDecryptHandleBytes32(options?.instance ?? this._fhevmEnv.instance, [{ handleBytes32, contractAddress: addr, fhevmType: mock_utils_1.FhevmType.eaddress }], user, options);
        if (!(handleBytes32 in decryptedResults)) {
    ...
    async getFHEVMConfig(contractAddress) {
        return (0, mock_utils_1.getFHEVMConfig)(this._fhevmEnv.readonlyEip1193Provider, contractAddress);
    }
    async getDecryptionOracleAddress(contractAddress) {
        return (0, mock_utils_1.getDecryptionOracleAddress)(this._fhevmEnv.readonlyEip1193Provider, contractAddress);
    }
    async assertCoprocessorInitialized(contract, contractName) {
        ...
        if (addresses.ACLAddress === ethers_1.ethers.ZeroAddress ||
            ...
            const errorMsg = `${errorMsgPrefix} is not initialized for FHE operations. Make sure it either inherits from @fhevm/solidity/config/${constants_1.default.FHEVM_CONFIG_SOLIDITY_FILE}:${constants_1.default.FHEVM_CONFIG_CONTRACT_NAME} or explicitly calls FHE.setCoprocessor() in its constructor.`;
            throw new error_1.HardhatFhevmError(errorMsg);
        }
        const addrMismatchErrorMsg = `${errorMsgPrefix} was initialized with FHEVM contract addresses that do not match the currently deployed FHEVM contracts. This is likely due to incorrect addresses in the file @fhevm/solidity/config/${constants_1.default.FHEVM_CONFIG_SOLIDITY_FILE}`;
...
async function _userDecryptHandleBytes32(instance, handleContractPairs, user, options) {
    return (0, mock_utils_4.userDecryptHandleBytes32)(instance, handleContractPairs, user, options);
}
