...
exports.getGatewayChainId = getGatewayChainId;
exports.getRelayerUrl = getRelayerUrl;
...
exports.getKMSVerifierAddress = getKMSVerifierAddress;
...
function getGatewayChainId() {
    return __getUintConstant("CHAIN_ID_GATEWAY", constants_1.default["CHAIN_ID_GATEWAY"]);
}
function getRelayerUrl() {
    return __getStringConstant("RELAYER_URL", constants_1.default["RELAYER_URL"]);
}
...
function getACLAddress() {
    const aclAddress = __getStringConstant("ACL_CONTRACT_ADDRESS", constants_1.default["ACL_CONTRACT_ADDRESS"], "addresses/.env.acl");
    if (!ethers_1.ethers.isAddress(aclAddress)) {
        throw new error_1.HardhatFhevmError(`Invalid ACL address ${aclAddress}`);
    }
    return aclAddress;
}
function getDecryptionOracleAddress() {
    const decryptionOracleAddress = __getStringConstant("DECRYPTION_ORACLE_ADDRESS", constants_1.default["DECRYPTION_ORACLE_ADDRESS"], "addresses/.env.decryptionoracle");
    if (!ethers_1.ethers.isAddress(decryptionOracleAddress)) {
        throw new error_1.HardhatFhevmError(`Invalid DecryptionOracle address ${decryptionOracleAddress}`);
    }
    (0, error_2.assertHHFhevm)(decryptionOracleAddress !== "0x33347831500F1e73f0ccCBb95c9f86B94d7b1123", 'decryptionOracleAddress !== "0x33347831500F1e73f0ccCBb95c9f86B94d7b1123"');
    (0, error_2.assertHHFhevm)(decryptionOracleAddress === "0xa02Cda4Ca3a71D7C46997716F4283aa851C28812", 'decryptionOracleAddress === "0xa02Cda4Ca3a71D7C46997716F4283aa851C28812"');
    return decryptionOracleAddress;
}
function getFHEVMExecutorAddress() {
    const fhevmExecutorAddress = __getStringConstant("FHEVM_EXECUTOR_CONTRACT_ADDRESS", constants_1.default["FHEVM_EXECUTOR_CONTRACT_ADDRESS"], "addresses/.env.exec");
    if (!ethers_1.ethers.isAddress(fhevmExecutorAddress)) {
        throw new error_1.HardhatFhevmError(`Invalid FHEVMExecutor address ${fhevmExecutorAddress}`);
    }
    return fhevmExecutorAddress;
}
function getInputVerifierAddress() {
    const inputVerifierAddress = __getStringConstant("INPUT_VERIFIER_CONTRACT_ADDRESS", constants_1.default["INPUT_VERIFIER_CONTRACT_ADDRESS"], "addresses/.env.inputverifier");
    if (!ethers_1.ethers.isAddress(inputVerifierAddress)) {
        throw new error_1.HardhatFhevmError(`Invalid InputVerifier address ${inputVerifierAddress}`);
    }
    return inputVerifierAddress;
}
function getKMSVerifierAddress() {
    const kmsVerifierAddress = __getStringConstant("KMS_VERIFIER_CONTRACT_ADDRESS", constants_1.default["KMS_VERIFIER_CONTRACT_ADDRESS"], "addresses/.env.kmsverifier");
    if (!ethers_1.ethers.isAddress(kmsVerifierAddress)) {
        throw new error_1.HardhatFhevmError(`Invalid KMSVerifier address ${kmsVerifierAddress}`);
    }
    return kmsVerifierAddress;
}
function getHCULimitAddress() {
    const address = __getStringConstant("HCULIMIT_CONTRACT_ADDRESS", constants_1.default["HCULIMIT_CONTRACT_ADDRESS"], "addresses/.env.hculimit");
    if (!ethers_1.ethers.isAddress(address)) {
        throw new error_1.HardhatFhevmError(`Invalid HCULimit address ${address}`);
    }
    return address;
}
