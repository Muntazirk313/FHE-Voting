...
(0, config_1.task)(task_names_1.TASK_CLEAN, async (_taskArgs, _hre, runSuper) => {
    ...
    try {
        if (fs.existsSync(fhevmEnv.paths.cache)) {
            ...
            debug(`${picocolors.greenBright(task_names_1.TASK_CLEAN)} remove directory ${fhevmEnv.paths.cache}`);
        }
        else {
            debug(`${picocolors.greenBright(task_names_1.TASK_CLEAN)} directory ${fhevmEnv.paths.cache} already removed.`);
        }
    }
    catch {
        console.log(`${constants_1.default.HARDHAT_PLUGIN_NAME}: Unable to remove directory '${fhevmEnv.paths.cache}'.`);
    }
    const res = await runSuper();
    return res;
});
