...
fhevmScope
    ...
    const fhevmEnv = EnvironmentExtender_1.fhevmContext.get();
    await fhevmEnv.minimalInit();
    const fhevmConfig = await (0, mock_utils_1.getFHEVMConfig)(hre.ethers.provider, address);
    if (fhevmConfig.ACLAddress === hre.ethers.ZeroAddress &&
    ...
    const decryptionOracleAddress = await (0, mock_utils_1.getDecryptionOracleAddress)(hre.ethers.provider, address);
    try {
        ...
        throw new error_1.HardhatFhevmError(`The contract deployed at ${address} is not using a valid FHEVM configuration`);
    }
});
fhevmScope
    ...
    const cfg = repo.getFhevmInstanceConfig({ chainId: fhevmEnv.chainId, relayerUrl: constants_1.default.RELAYER_URL });
